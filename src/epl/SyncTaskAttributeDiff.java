...
/**
 * Copied from Mylyn TaskAttributeDiff which compares attributes by their labels in their option maps, not their values.
 * Sync requires that attributes are compared by their actual values.
 */

public class SyncTaskAttributeDiff implements ITaskAttributeDiff {
	...
	public SyncTaskAttributeDiff(TaskAttribute oldAttribute, TaskAttribute newAttribute) {
		Assert.isTrue(oldAttribute != null || newAttribute != null);
		this.oldAttribute = oldAttribute;
		this.newAttribute = newAttribute;
		if (oldAttribute != null) {
			this.oldValues = oldAttribute.getTaskData().getAttributeMapper().getValues(oldAttribute);
			this.attributeId = oldAttribute.getId();
		} else {
			this.oldValues = Collections.emptyList();
		}
		if (newAttribute != null) {
			this.newValues = newAttribute.getTaskData().getAttributeMapper().getValues(newAttribute);
	...
	public boolean equals(Object obj) {
		...
		SyncTaskAttributeDiff other = (SyncTaskAttributeDiff) obj;
		if (attributeId == null) {
